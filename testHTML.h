...
/*
 * testHTML.h : a small tester program for HTML input.
 *
...
// 2022-01-25 converted from testHTML.c 
...
//g++  -DTEST_TESTHTML_MJM -std=c++11 -I/usr/include/libxml2 -I../../mjm/hlib  -x c++ testHTML.h -o testHTML  -lxml2
...
// polluting the global names space doh 
namespace testHTML {
...
static void hier_properties_stack (Ragged & r,  xmlAttr*  p, IdxTy flags,  int lvl, Ragged::Line rl );
////////////////////////////////////////////////////////////

#define Bit(f,i) (((f>>i)&1)!=0)

 void hier_element_names_stack(Ragged & r, xmlNode * a_node, IdxTy flags, int lvl, Ragged::Line rl )
{
const bool only_json_vals=Bit(flags,0);
//char * cb= new char[1<<24];
const Ragged::Line rlzed=rl;
//printf(" xxx loc b\n");
...
//	char px[msz];
	const unsigned char*  px=p; // [msz];

int ok_to_print=0;
       //sprintf(cb,"%s %s %d %s %s",pfx.c_str(),c, lvl,cur_node->name,px);  

// 2022-10-30 see if this is the diff not parsing a nullscript
// original 
if ( p) if (p[0]) ok_to_print=1;
//if ( p)  ok_to_print=1;

ok_to_print|=!(cur_node->children||cur_node->properties); 
...
//r<<ss.str();
rl.push_back(ss.str());
//if (cur_node->name ) r<<cur_node->name; else r<<"";
if (cur_node->name ) rl.push_back((const char *) cur_node->name); 
else rl.push_back("");
// sprintf(cb,"%s%d %s = %s",pfx.c_str(), noden,cur_node->name,px);  
if (ok_to_print) { 
//Ragged::Line here=r[r.size()-1];
//MM_ERR(MMPR2(here.size(),r.size()))
//if (p) r<<px;  else r<<"";
Ragged::Line x=rl;
if (p) rl.push_back((const char*)px);  else rl.push_back("");
// this line may be spurious 
//r.empty_line(); 
// 2022-10-30
bool ok_to_add=true;
//bool ok_to_add=!true;
...
{
ok_to_add=false;
const char * pv=rl.back().c_str();
// 2022-10-30 comment out 
while (*pv) { if (*pv=='{') {ok_to_add=true; break;}++pv; }
}
if (ok_to_add) r.add(rl);
rl=x;
// it can't do this until it find the right place in hierarching.. 
//r.add(here);
} // ok_to_print
       	//print_properties_stack(pfx2,cur_node->properties,fsp,lvl+1); 
       	hier_properties_stack(r,cur_node->properties,flags,lvl+1,rl); 
        //print_element_names_stack(pfx2,cur_node->children,fsp, lvl+1);
        hier_element_names_stack(r,cur_node->children,flags, lvl+1,rl);
	} // for
//delete []cb;
} // hier_element_names_stack
/////////////////////
...
hier_element_names_stack(r,c->children,flags, lvl+1,rl);
}

//delete [] cb;

} // hier_properties_stack
...
// this is the internally called code  from parse_stram
IdxTy hier_doc(Ragged & r, htmlDocPtr doc, const IdxTy flags)
{
  xmlNode *root_element = xmlDocGetRootElement(doc);

 hier_element_names_stack(r,root_element,flags,0,Ragged::Line()); 
return 0; 
} // hier_doc
	    ...
	    xmlElementContentPtr content)
{
    fprintf(stdout, "SAX.elementDecl(%s, %d, ...)\n",
            name, type);
// k this is toin to take foruck forever for this   
...
IdxTy parse_stream(Ragged & r, IsTy & is, const IdxTy flags)
{
    //htmlDocPtr doc = NULL, tmp;
    htmlDocPtr doc = NULL; // compiled unused var
const char * filename="dummynamewtf.html";
	    int res; // , size = 3;
const IdxTy bsz=1<<16;
	    char chars[bsz];
	    htmlParserCtxtPtr ctxt;
	//	size = bsz; // 4096;
	    //res = fread(chars, 1, 4, f);
		...
		ctxt = htmlCreatePushParserCtxt(NULL, NULL,
			    //chars, res, filename, XML_CHAR_ENCODING_NONE);
			    chars, res, filename, XML_CHAR_ENCODING_UTF8);

//		while ((res = fread(chars, 1, size, f)) > 0) { htmlParseChunk(ctxt, chars, res, 0); }
		while (true)
		{
		if (is.eof()) break; 
	    is.read(chars,bsz);
		res=is.gcount();
		if (res==0){
			MM_ERR(" blocking on "<<MMPR(res))
			sleep(1);
			continue; } //  break; // blocking for more or eof?
		htmlParseChunk(ctxt, chars, res, 0); 
		} // true
...
//	    fclose(f);
hier_doc(r,doc,flags); 
return 0; 
}  // parse_stream
void parseAndPrintFile(char *filename) {
if (strcmp(mjm_sflags,"rag")==0) 
{
Ragged r;
// compier unused
IdxTy rc=0;
...
{
rc= parse_stream( r,  std::cin, 0);
}
else
{
std::ifstream is(filename);
rc= parse_stream( r,  is, 0);
}

MM_ERR(MMPR(rc)) // compiler complains unused lol 
MM_MSG(r.dump())
return ;
} 
    htmlDocPtr doc = NULL, tmp;
...
#ifdef TEST_TESTHTML_MJM 
using namespace testHTML;
	...
	{ ++i; if (i<argc) 
{
	...
	else if ((!strcmp(argv[i], "-rag")) || (!strcmp(argv[i], "--rag")))
{
++i; if (i<argc)
{
IdxTy flags=0;
if ((i+1)<argc) { flags=atoi(argv[i+1]); } 
//IdxTy parse_stream(Ragged & r, IsTy & is, const IdxTy flags)
Ragged r;
std::ifstream is(argv[i]);
IdxTy rc=parse_stream(r,is,flags);
MM_ERR(r.dump())
return 0; 
break;
} // argc
