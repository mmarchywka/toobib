...
//#define KVP(x) kvp.get(x,QUOTE(x)); 
...
template <class Tv> void split(Tv & v,const StrTy & x, const char cep)
{
...
while (s[pc])
{
if (s[pc]==cep) { s[pc]=0; v.push_back(StrTy(s+start)); start=pc+1; }
++pc;
} // pc 
if( start!=pc) v.push_back(StrTy(s+start)); 
