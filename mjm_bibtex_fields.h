...
if (vz==5) // yyyy-mm-dd  or mm-dd-yyyy
{
if (v[0].length()==4)if(sepp(v[1])) if (v[2].length()==2) if (sepp(v[3]))
{
MM_LOOP(ii,v)  m_s+=(*ii); 
 m_y=v[0];
m_m=v[2];
...
if (v[4].length()==4)if(sepp(v[1])) if (v[2].length()==2) if (sepp(v[3]))
{
MM_LOOP(ii,v)  m_s+=(*ii); 
 m_y=v[4];
m_m=v[0];
...
{
const Line & lin=(*jj);
const IdxTy llen=lin.size();
Line lout;
MM_ILOOP(k,llen)
{
const StrTy & w=lin[k];
const bool concat=(w=="and");
if (!concat) {  lout.push_back(w); continue; } 
// remove any spaces first ...
if (lout.size()==0) continue;
if (lout[lout.size()-1]==" ") lout.pop_back();
if ((k+1)<llen) { ++k; if (lin[k]==" ") ++k; }
if (lout.size()){  r2.add(lout); lout.clear(); } 
} // k 
if (lout.size()){ r2.add(lout); lout.clear(); } 
