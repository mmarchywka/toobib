...
// this seems to be called from 2 places- json and html parsers
// with different key placements.
// json uses sane=0 for now html now uses sane=1
IdxTy Organize( hier_map & hm,const Ragged & r, const StrTy & key,const IdxTy flags) 
{
const bool sane_key=Bit(flags,0);
MM_ERR(MMPR3(key,flags,sane_key))
const IdxTy npos=2;
const IdxTy npos2=4;
const IdxTy wpos=3;
const IdxTy vpos=1;
...
{
//MM_ERR(MMPR2(nm,l[j]))
if (sane_key) hm[l[j-1]][l[len-npos2]]=l[len-vpos];
else hm[l[len-wpos]][l[len-npos]]=l[len-vpos];
//MM_ERR(MMPR4(nm,l[j],l[j-1],l[len-4])<<MMPR(l[len-1]))
MM_ERR(MMPR3(__FUNCTION__,l[j-1],l[len-npos2])<<MMPR3(l[j],l[len-wpos],l[len-npos])<<MMPR(l[len-vpos]))
